...
# This script generates a plot of ANOSIM results. ANOSIM were performed on Bray-Curtis distances across cereal flours (n=19) and couscous (n=19) related to metadata.
...
metadata2 <- metadata %>%
  ...
  select(c("clean_id", "ferm", "cereals"))
...
bray_dist <- phyloseq::distance(phyloseq_template, method = "bray")
...
# Export Excel
#write_xlsx(results_anosim, "SD12_fig4b_anosim_beta_div_couscous.xlsx")
...
ggsave(
  filename = "plots/SD12_fig4b_anosim_beta_div_couscous.png", 
  plot = last_plot(),             
  width = 88,                      
  height = 60,                     
  units = "mm",
...
ggsave(
  filename = "plots/SD12_fig4b_anosim_beta_div_couscous.svg", 
  plot = last_plot(),              
  width = 88,                     
  height = 60,                     
  units = "mm"
