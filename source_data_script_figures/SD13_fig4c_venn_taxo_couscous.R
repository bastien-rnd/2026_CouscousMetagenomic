...
# This script generates Venn diagram of genes in cereal flours (n=19) and couscous (n=19) according to taxonomic table (species-level).
...
samples_yes <- metadata_f %>% filter(ferm == "yes") %>% pull(clean_id)
samples_no  <- metadata_f %>% filter(ferm == "no") %>% pull(clean_id)
...
venn_data <- list(`Cereals flours` = species_no, `Couscous` = species_yes)

ggvenn(
  venn_data,
  fill_color = c("#86BF94", "#E69F00"),
  stroke_size = 0.3,
  set_name_size = 7 / .pt,
  text_size = 5 / .pt,
...
# Export plot
ggsave(
  filename = "plots/SD13_fig4c_venn_taxo_couscous.png",  
  plot = last_plot(),   
  width = 40,
  height = 40,
  units = "mm",            
...
ggsave(
  filename = "plots/SD13_fig4c_venn_taxo_couscous.svg", 
  plot = last_plot(),         
  width = 40,
  height = 40,
  units = "mm"
...
write_xlsx(venn_table, "SD13_fig4c_venn_taxo_couscous.xlsx")
