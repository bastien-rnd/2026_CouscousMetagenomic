...
# This script generates PCoA of Beta-diversity across fermented cereals (n=94)
...
bray_curtis_dist_template <- distance(phyloseq_template, method = "bray")
bray_curtis_mat <- as.matrix(bray_curtis_dist_template)
bray_curtis_df <- data.frame(Sample = rownames(bray_curtis_mat),
                             bray_curtis_mat,
                             check.names = FALSE)
#write_xlsx(bray_curtis_df, "SD4_fig2b_beta_div_fermented_cereals.xlsx")
...
# PLOT
colors_vector <- colors[["product"]]

plot_ordination(phyloseq_template, pcoa_result_template, color = "product") +
  geom_point(size = 0.5, stroke = 0.5) + 
  stat_ellipse(aes(group = couscous), type = "norm", linetype = "solid", size = 0.5, alpha = 0.3) +
...
# To export plot
ggsave(
  filename = "plots/SD4_raw_fig2b_beta_div_fermented_cereals.png",
  plot = last_plot(),
  width = 121,
  height = 70,
  units = "mm",
...
ggsave(
  filename = "plots/SD4_raw_fig2b_beta_div_fermented_cereals.svg",
  plot = last_plot(),
  width = 121,
  height = 70,
  units = "mm"
